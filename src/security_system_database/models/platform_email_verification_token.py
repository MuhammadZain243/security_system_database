"""Platform email verification token model."""

from datetime import datetime
from uuid import UUID

from sqlalchemy import ForeignKey, Index, String, UniqueConstraint, func
from sqlalchemy.orm import Mapped, mapped_column

from security_system_database.base import Base
from security_system_database.models.mixins import UUIDPrimaryKeyMixin
from security_system_database.models.types import DATETIME_TIMEZONE, UUID_TYPE


class PlatformEmailVerificationToken(Base, UUIDPrimaryKeyMixin):
    """Short-lived email verification token for a platform user."""

    __tablename__ = "platform_email_verification_tokens"
    __table_args__ = (
        UniqueConstraint(
            "token_hash",
            name="uq_platform_email_verification_tokens_token_hash",
        ),
        Index("ix_platform_email_verification_tokens_expires_at", "expires_at"),
        Index(
            "ix_platform_email_verification_tokens_platform_user_id",
            "platform_user_id",
        ),
    )

    platform_user_id: Mapped[UUID] = mapped_column(
        UUID_TYPE,
        ForeignKey(
            "platform_users.id",
            name="fk_platform_email_verification_tokens_platform_user_id",
            ondelete="RESTRICT",
        ),
        nullable=False,
    )
    token_hash: Mapped[str] = mapped_column(String(255), nullable=False)
    expires_at: Mapped[datetime] = mapped_column(DATETIME_TIMEZONE, nullable=False)
    used_at: Mapped[datetime | None] = mapped_column(
        DATETIME_TIMEZONE,
        nullable=True,
    )
    created_at: Mapped[datetime] = mapped_column(
        DATETIME_TIMEZONE,
        server_default=func.now(),
        nullable=False,
    )
